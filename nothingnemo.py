...
BOT_TOKEN = "8791634150:AAEDRdDmhSuHgrCYSRYDk6NLoRSYI8p1NDk"
ADMIN_IDS = [6258211515]
...
def start_cmd(m):
    ...
    kb.add(InlineKeyboardButton("🎮 DEMO", url=demo_link))
    ...
    try:
        ...
        if price == 0:
            ...
            bot.send_message(
                ...
                text=f"🔍 *DEMO PRODUCT*\n\n{reward}\n\nThis is just a preview. To get real premium plan, please select a paid plan.",
                reply_markup=kb,
                parse_mode="Markdown"
            )
            bot.answer_callback_query(call.id)
...
def paid_cb(call):
    try:
        ...
        dup_count = int(get_setting('duplicate_multiplier'))
        if existing:
...
def screenshot_handler(m):
    ...
    admin_msg = (
        ...
        f"👤 *User:* {username}\n"
        f"🆔 *User ID:* `{user.id}`\n"
        f"📞 *Contact:* [Click here]({user_link})\n"
        f"📦 *Product:* {pname}\n"
        f"💰 *Amount:* ₹{amt}\n"
        f"🆔 *Payment ID:* `{pay_id}`"
    )
...
"""
    ...
    stats = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⚙️ Duplicate Multiplier: {dup_mult}x\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━"
    )
...
def payment_details(call):
    ...
    for p in payments:
        ...
        msg += f"🆔 {p[0]} | 👤 {p[1]} | {p[2]} | ₹{p[3]} | {status_icon} | x{p[5]}\n🕐 {p[5]}\n\n"
...
def set_demo_cb(call):
    if not is_admin(call.from_user.id): return
    bot.send_message(call.message.chat.id, "🎮 *SET DEMO LINK*\n\nSend invite link:\n`https://t.me/yourchannel`", parse_mode="Markdown")
...
def save_demo(m):
    ...
    if "t.me" in link or "http" in link:
        ...
        bot.send_message(m.chat.id, "✅ Demo link updated!")
    else:
